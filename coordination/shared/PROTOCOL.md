...
The shared local repository at `C:\Studio_Dev\gielinor-cardbound` is the synchronization bridge and Codex engineering workspace. ChatGPT and Codex can both inspect and update it directly. GitHub remains the source-control, PR, and CI remote; it is not the handoff transport.

The testing app remains hosted through GitHub for Setetchie and future external testers. Local coordination completion and tester-build publication are separate events: ordinary handoffs stay local, while a task that creates or updates a tester-visible build must explicitly include GitHub publication/deployment in its authorized scope.
...
3. Read `AGENTS.md`, this protocol, `WORKFLOW_STATE.md`, and `coordination/initiate/CURRENT.md` directly from the shared checkout.
4. Reconcile the task with the current local worktree and preserve unrelated changes.
5. Fetch remote state only when the assigned work requires current GitHub branch, PR, or CI information.
6. If remote integration is required, compare divergence and changed paths, then pull with fast-forward-only semantics only when it cannot overwrite or conflict with local work.
7. Stop and report when histories diverge, the upstream is missing, or remote changes conflict with local work. Never automatically reset, rebase, stash, discard, overwrite, or destructively resolve unrelated work.
...
- For tester-build tasks: target branch/environment, required commit/push or deployment action, visible build/version identifier, test URL, and deployment verification.
...
- Local mailbox synchronization does not authorize committing, pushing, merging, force-pushing, rebasing, resetting, discarding unrelated changes, overwriting uncommitted work, inventing product/design decisions, authorizing 50× testing, or inferring merge approval.
