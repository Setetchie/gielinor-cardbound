...
| OPS-002 | Validation | Diagnose/patch, run a complete 1× gate, fix and restart if red, and run 50× only after fully green 1×. | `CURRENT` | v44 review board, Testing process/current action state | Supersedes run #74 workflow | Satisfied: 1× 37/37 and fresh 50× 1,850/1,850 green | Any later code change resets both validation gates. |
| OPS-003 | Validation state | Run #74/ID `32073559950`: five named shards failed and `v43-structure` was cancelled. | `SUPERSEDED` | v44 review board; `docs/CARDBOUND_NEW_CHAT_HANDOFF_2026-08-18.md` | Superseded by `OPS-004` | Root causes repaired in the working tree | Historical failure evidence: `prototype-core`, `v44-ux`, `greenwake-progression`, `greenwake-terminology`, `prototype-ui`; `v43-structure` timed out on its incompatible bootstrap fixture. |
| OPS-004 | Validation state | The corrective working tree passed the complete 1× Cardbound gate and one fresh complete 50× gate. | `CURRENT` | Setetchie's 2026-08-18 approvals; `coordination/results/CB-PR5-STRESS50-005.md` | Supersedes `OPS-003` | Green: 37/37 at 1× and 1,850/1,850 at 50× | 1× completed in 17.4s; fresh 50× completed across all 11 shards in 1,384.9s with no failures, flakes, skips, retries, or hangs. |
...
| TERM-003 | Terminology/economy | Current player-facing Forge currencies are universal normal Fragments, Pristine Fragments for Foil/Perfect duplicates, and Star Fragments. | `CURRENT` | Setetchie's v45 owner feedback `V45-OWNER-010`; Setetchie's 2026-08-20 instruction to freeze and begin implementation | Supersedes the `Prismatic Essence` portion of `TERM-002`; retains Bank, Forge, universal Fragments, and Star Fragments | Approved for the next scoped implementation batch | Exact conversion yields, costs, odds, and Star Fragment effects remain `BALANCE`; do not invent them. |
